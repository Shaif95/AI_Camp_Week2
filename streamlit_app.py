...
st.plotly_chart(
  px.bar(taxable_quantiles, x="Quantile", y=)
)
...
st.heading("Hypothesis: Which Quartile Has The Highest Taxes?")

mean_summary_index = df.groupby('Quantile')['Taxes'].mean().reset_index()

fig = px.bar(mean_summary_index, x='Quantile', y='Taxes', color='Quantile',
